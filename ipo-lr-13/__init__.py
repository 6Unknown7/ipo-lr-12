...
class Vehicle:
    def __init__(self, capacity: float):
        ...
        self.vehicle_id = str(uuid.uuid4())[:8]  # Генерируем уникальный ID
    ...
    def __str__(self):
        return (f"Транспорт ID: {self.vehicle_id}, "
                ...
                f"Свободно: {self.capacity - self.current_load}т")class Truck(Vehicle):
...
class TransportCompany:
    ...
    def optimize_cargo_distribution(self):
        ...
        # Сортируем транспорт по грузоподъемности (по убыванию)
        sorted_vehicles = sorted(self.vehicles, key=lambda x: x.capacity, reverse=True)
        
        # Копируем клиентов для обработки
        clients_to_process = sorted_clients.copy()
        
        for vehicle in sorted_vehicles:
            # Сбрасываем текущую загрузку транспорта
        ...
        for client in clients_to_process:
            ...
            # Если не поместился в существующий, ищем новый транспорт
